...
iter_loc = 110; %logdata.data(1,iter_loc) : number of state points in the file i
alpha_loc=69;
...
for i = 1:a
    alpha(i) = logdata.data(1,alpha_loc);
end
...
for i = 1:a
    for j = 118:(117+logdata.data(1,iter_loc))
        Xc_rand(i,j) = logdata.data(1,j);
    end
end
 
for i = 1:a
for j = (118+logdata.data(1,iter_loc)):(iter_loc+logdata.data(1,iter_loc)+logdata.data(1,iter_loc))
G_rand(i,j) = logdata.data(1,j);
end
end
...
for j = 1:a
    for i = 1:(logdata.data(ramp_loc1,iter_loc))
        Xc(:,i,j) = data0(:,(1+i),j);
        Xd(:,i,j) = data0(:,(1+logdata.data(1,iter_loc)+i),j);
       Delta(:,i,j) = data0(:,(1+2*logdata.data(1,iter_loc)+i),j);
%        Gain(:,i,j) = data0(:,(1+3*logdata.data(1,iter_loc)+i),j);
    end
end
    ...
    case 3          % Discrete Frequencies
        ...
        switch choice2
            ...
            case 2
                ...
                freq_stim=[5	9	13	17	21	24	27	30	40	60	80];     %%%% INPUT FREQUENCY ARRAY %%%%
            ...
            case 4
                ...
                amp_stim=[0.01	0.03	0.1	0.3	1	3	10	20	50	100	300	500];     %%%% INPUT AMPLITUDE ARRAY %%%%
                ...
                stim_time(1:numfreq)=cycles*Fs*1e-3;
                ...
                for i = 1:(logdata.data(1,iter_loc))
                    ...
                    if p>1
                        clear stim_pad;
                        stim_pad = Xd((delay1*p+sum(stim_time(1:p-1)))-offset:(delay1*p+sum(stim_time(1:p)))-offset,i,j);
                        input_pad = Xc((delay1*p+sum(stim_time(1:p-1)))-offset:(delay1*p+sum(stim_time(1:p)))-offset,i,j);
                        Delta_pad = Delta((delay1*p+sum(stim_time(1:p-1)))-offset:(delay1*p+sum(stim_time(1:p)))-offset,i,j);
                        stim_pad(length(stim(:,1,1,1)))=0;
                        input_pad(length(stim(:,1,1,1)))=0;
                        Delta_pad(length(stim(:,1,1,1)))=0;
                        stim(:,p,i,j)=stim_pad;
                        ...
                        stim(:,p,i,j)=Xd((delay1*p)-offset:(delay1*p+stim_time(p)-offset),i,j);
                        input(:,p,i,j)=Xc((delay1*p)-offset:(delay1*p+stim_time(p)-offset),i,j);
                        Delta_in(:,p,i,j)=Delta((delay1*p)-offset:(delay1*p+stim_time(p)-offset),i,j);
                    end
                end
                end
                end
...
for i = 1:(logdata.data(1+ramp_loc1-1,iter_loc))
for p = 1:numfreq
h_stim=psd(spectrum.periodogram('blackman'),stim_center(:,p,i,j),'Fs',Fs,'NFFT',1e4);
...
stim_amp(p,i,j)=sqrt(psd_q2_stim(q2,p,i,j)*psd_q1_stim(q2,p,i,j))/2;       % Output peak amplitude @ stimulus frequency
...
force_in(p,i,j) = (ksf + sqrt(-1)*freq_stim(p)*gsf)*input_fft1(p,i,j);
...
L = length(stim_center(:,p,i,j));
hfft  = fft(stim_center(:,p,i,j),NFFT)/L;
qfft_stim(:,2)=hfft(1:NFFT/2+1);
qfft_stim(:,1) = Fs/2*linspace(0,1,NFFT/2+1);
...
q = find((freq_stim-1)<psd_q1_stim(:,p,i,j)&1.05*(freq_stim+1)>psd_q1_stim(:,p,i,j));
%q = q-2:1:q+2;
q2 = q(find(psd_q2_stim(q,p,i,j)==max(psd_q2_stim(q,p,i,j))));
...
for i = 1:(logdata.data(1+ramp_loc1-1,iter_loc))
for p = 1:numfreq
h_stim=psd(spectrum.periodogram('blackman'),Delta_in_center(:,p,i,j),'Fs',Fs,'NFFT',2e4);

L = length(Delta_in_center(:,p,i,j));
hfft  = fft(Delta_in_center(:,p,i,j),NFFT)/L;
qfft_Delta(:,2)=hfft(1:NFFT/2+1);
qfft_Delta(:,1) = Fs/2*linspace(0,1,NFFT/2+1);
...
% INPUT
ksf = 250e-6;       % Fiber stiffness (N/m)
gsf = 200e-9;       % Fiber damping (Ns/m)
amp = 5e-9;         % stimulus amplitude (m)
...
for i = 1:(logdata.data(1+ramp_loc1-1,iter_loc))
for p = 1:numfreq
sensitivity(p,i,j)=stim_amp(p,i,j)/(sqrt(ksf^2+(gsf*freq_stim*2*pi)^2)*amp_stim(p)*1e-9)*1e-9;
...
for p = 1:numfreq
    ...
    figure(2*a+j)
    subaxis(logdata.data(1,iter_loc),numfreq,p+(i-1)*numfreq,'Spacing', 0.01, 'Padding', 0, 'Margin', 0);
...
if choice == 5
    ...
    disp('Incorrect Choice');offset
end


%%  COMPRESSION PLOT FIG 3

for i = 1:8
for j = 1:4
meansens(i,j)=mean_stim(i,j)/(amp_stim(i)*k_rand(j));
semsens(i,j)=sem_stim(i,j)/(amp_stim(i)*k_rand(j));
end
end
