...
Npt = (Ntrial-1)*Np+Npulse;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if Tstartend(1,Npulse) ~= Tstartend(2,Npulse)
    ssstartpt = find(abs(tvec-Tstartend(1,Npulse))==min(abs(tvec-Tstartend(1,Npulse))));
    ssendpt = find(abs(tvec-Tstartend(2,Npulse))==min(abs(tvec-Tstartend(2,Npulse))));
    else
    ssstartpt = 1;
    ssstartpt = round(length(Xd)/2);
    ssendpt = length(Xd);
...
%wsdetrend should be independent of each tt length
wsdetrend = round(ws);
X = Xd(ssstartpt:ssendpt,Npulse)- smooth(Xd(ssstartpt:ssendpt,Npulse),wsdetrend,'moving');
...
plot(tvec(ssstartpt:ssendpt),X(ssstartpt:ssendpt),'g')
if Tstartend(1,Npulse) ~= Tstartend(2,Npulse)
text(0.5*tmax,0.8*Xmax,{[num2str(Tstartend(1,Npulse)/1000,'%3.1f') '-' num2str(Tstartend(2,Npulse)/1000,'%3.1f') ' s']},...
    'FontSize',12,'HorizontalAlignment','center');
end
axis([tmin,tmax,Xmin,Xmax])
text(tmax/2,0.6*Xmin,{'Local RMS' [num2str(Xmeanstd,'%4.1f') 'nm']},...
