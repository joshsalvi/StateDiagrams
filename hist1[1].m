...
XdRSAVec = XdRSA;
XdRSAVec(end) = [];
figure; quiver(real(XdRSAVec),imag(XdRSAVec),real(DXdRSA),imag(DXdRSA));
...
TestTime = 0:TestT/1000:100*TestT;
...
TestSigR = sawtooth(w.*TestTime,0.5) + 0.1.*randn(length(TestTime),1)';
TestSig = hilbert(TestSigR);

%Create a vector plot
DTestSig = diff(TestSig)./(timeR(2)-timeR(1));
TestSigVec = TestSig;
TestSigVec(end) = [];
figure; quiver(real(TestSigVec),imag(TestSigVec),real(DTestSig),imag(DTestSig));
...
BinSizeR = 2*iqr(real(TestSig))/length(TestSig)^(1/3);
NRbins = floor((maxR - minR)/BinSizeR);
BinSizeI = 2*iqr(imag(TestSig))/length(TestSig)^(1/3);
NIbins = floor((maxI - minI)/BinSizeI);
